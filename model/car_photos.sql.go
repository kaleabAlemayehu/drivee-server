...
UPDATE car_photos cp SET photo_url=$3 FROM cars c WHERE cp.car_id = c.id AND cp.id =$1 AND c.owner_id = $2 RETURNING cp.id, cp.car_id, cp.photo_url, cp.created_at, cp.updated_at
`
